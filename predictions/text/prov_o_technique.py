...
counter_pred = 0
counter_act = 0
for file_name in glob.glob('technique.csv'):
    ...
    for index, row in x.iterrows():
        ...
        if predicted == "http://data.silknow.org/vocabulary/facet/damask":
           predicted = "http://data.silknow.org/vocabulary/168"
        if predicted == "http://data.silknow.org/vocabulary/facet/embroidery":
           predicted = "http://data.silknow.org/vocabulary/87"
        if predicted == "http://data.silknow.org/vocabulary/facet/velvet":
        ...
        m = """}
           ...
           BIND(URI(REPLACE(CONCAT(STR(?object), "/text/technique/"""
        n = str(counter_pred)
