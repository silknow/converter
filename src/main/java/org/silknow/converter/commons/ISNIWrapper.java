...
public class ISNIWrapper {
  ...
  public static void init(String isni_folder) {
    folder = isni_folder;
  ...
  public static ISNIRecord get(String isni) throws IOException {
    if (folder == null) throw new RuntimeException("isni folder not set");
  ...
  public static void main(String[] args) throws IOException {
    File isniFolder = Paths.get("temp", "isni").toFile();
    isniFolder.mkdirs();
    ISNIWrapper.init(isniFolder.getAbsolutePath());

    ISNIWrapper.search("Zhang, Lu 張路", "1464");
  }
}
